...
const REGEX_MODULE_INDEX = /^(\/modules\/[\w-]+)*\/index\.vue$/i
const REGEX_VALIDATE_DIR = /^\/(modules\/[\w-]+\/)*(modules(\/[\w-]+|)|pages)$/i
const REGEX_VALIDATE_PAGE = /^(\/modules\/[\w-]+)*\/pages\/[\w-]+\.vue$/i
...
function createRoutes (files: { [key: string]: string }, options: RouteOptions) {
  ...
  Object.keys(files).sort().forEach(function (file) {
    let keys = file.replace(/\.vue$/, '').replace(/\/{2,}/g, '/').split('/').slice(1)
    let route = {
      name: '',
      path: '',
      ...
      hasExOptions: config.PICK_REGEX.test(fs.readFileSync(files[file]) + '')
    }
    let parent = routes
