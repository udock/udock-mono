...
const macProxyManager = {
  ...
  enableGlobalProxy (ip: string, port: number, proxyType: string) {
    if (!ip || !port) {
  ...
  disableGlobalProxy (proxyType: string) {
    proxyType = proxyType || 'http'
    const networkType = macProxyManager.networkType || macProxyManager.getNetworkType()
    return /^http$/i.test(proxyType) ?
...
const winProxyManager = {
  ...
  enableGlobalProxy (ip: string, port: number) {
    if (!ip && !port) {
      console.log('failed to set global proxy server.\n ip and port are required.')
      return
    }
  ...
  disableGlobalProxy: () => execSync('reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings" /v ProxyEnable /t REG_DWORD /d 0 /f'),
