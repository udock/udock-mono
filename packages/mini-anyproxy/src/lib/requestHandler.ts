...
import * as requestErrorHandler from './requestErrorHandler'
import Recorder from './recorder';
import { ThrottleOptions } from 'stream-throttle';
...
*/
const getErrorResponse = (error: Error, fullUrl: string) => {
  ...
  const errorResponse = {
    ...
    body: requestErrorHandler.getErrorContent(error as any, fullUrl)
  }
 ...
 * @returns
 */
function fetchRemoteResponse(protocol: string, options: any, reqData: string, config: any) {
  reqData = reqData || ''
  return new Promise((resolve, reject) => {
    ...
    const proxyReq = (/https/i.test(protocol) ? https : http).request(options, (res) => {
      ...
      let resDataChunks: Uint8Array[] = []; // array of data chunks or stream
      const rawResChunks: string[] = []; // the original response chunks
      ...
      const finishCollecting = () => {
        new Promise((fulfill, rejectParsing) => {
          if (resDataStream) {
            ...
            const contentEncoding = resHeader['content-encoding'] || (resHeader['Content-Encoding'] as string)
...
*
*/
function getWsReqInfo(wsReq: any) {
 ...
 * @returns
 */
function getUserReqHandler(this: RequestHandler, userRule: any, recorder: Recorder) {
  const reqHandlerCtx = this
  return function (req: any, userRes: any) {
    /*
    note
      req.url is wired
    ...
    */
    ...
    logUtil.printLog(chalk.green(`received request to: ${req.method} ${host}${path}`))
    ...
    */
    const sendFinalResponse = (finalResponseData: any) => {
      ...
      if (global._throttle) {
        if (responseBody instanceof CommonReadableStream) {
          responseBody.pipe(global._throttle.throttle({} as ThrottleOptions)).pipe(userRes)
        } else {
          const thrStream = new Stream()
          thrStream.pipe(global._throttle.throttle({} as ThrottleOptions)).pipe(userRes)
    ...
    fetchReqData()
      ...
      .then(async () => {
        ...
        const finalReqDetail: any = {}
        ;['protocol', 'requestOptions', 'requestData', 'response'].map((key: string) => {
      ...
      .then(async (userConfig) => {
        if (userConfig.response) {
          ...
          const remoteResponse: any = await fetchRemoteResponse(userConfig.protocol, userConfig.requestOptions, userConfig.requestData, {
      ...
      .catch((e) => {
        logUtil.printLog(chalk.green('Send final response failed:' + e.message), logUtil.T_ERR)
      })
  }
}
 ...
 * @returns
 */
function getConnectReqHandler(this: any, userRule: any, recorder: Recorder, httpsServerMgr: HttpsServerMgr) {
  const reqHandlerCtx = this;
  reqHandlerCtx.conns = new Map();
  reqHandlerCtx.cltSockets = new Map()

  return function (req: any, cltSocket: any, head: any) {
    ...
    */
    ;(async () => {
      ...
      logUtil.printLog(chalk.green('received https CONNECT request ' + host))
      requestDetail = {
        host: req.url,
        _req: req
      }
    ...
    .then(() => {
      ...
      if (!shouldIntercept) {
        ...
        return httpsServerMgr.getSharedHttpsServer(host).then((serverInfo: any) => ({ host: serverInfo.host, port: serverInfo.port }))
      }
    })
    .then((serverInfo: any) => {
      ...
      return new Promise((resolve, reject) => {
        const conn = net.connect(serverInfo.port, serverInfo.host, () => {
          ...
          if (global._throttle && !shouldIntercept) {
            ...
            conn.pipe(global._throttle.throttle({} as ThrottleOptions)).pipe(cltSocket)
          } else {
            requestStream.pipe(conn)
            conn.pipe(cltSocket)
          }
...
*/
function getWsHandler(this:RequestHandler, userRule: any, recorder: Recorder, wsClient: any, wsReq: any) {
  const self = this
  try {
    let resourceInfoId = -1
    ...
    if (recorder) {
      resourceInfoId = recorder.appendRecord({
        ...resourceInfo,
    ...
    */
    const sendProxyMessage = (event: any) => {
    ...
    */
    const getCloseFromOriginEvent = (event: any) => {
    ...
    *
    */
    const recordMessage = (messageEvent: any, isToServer: boolean) => {
      const message = {
        time: Date.now(),
...
export default class RequestHandler {
  ...
  wsIntercept: boolean
  connectReqHandler: Function

  userRequestHandler?: Function
  wsHandler?: Function
  httpsServerMgr?: HttpsServerMgr
   ...
   */
  constructor(config: any, rule: any, recorder: Recorder) {
    const reqHandlerCtx = this
